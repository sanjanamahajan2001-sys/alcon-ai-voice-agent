...
async function fetchLeads() {
    try {
        ...
        if (leads.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" style="text-align: center; padding: 40px; color: var(--text-muted);">No campaign data yet. Start a call!</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = leads.map(lead => `
            <tr>
                ...
                <td style="font-size: 0.85rem; color: var(--primary-gold);">
                    ${lead.next_step || 'Wait'}
                ...
                <td>
                    <div style="display: flex; gap: 5px;">
                        ...
                        ${lead.escalation_status === 'REQUIRED' ? `
                            <button class="action-btn-mini" style="background: #ef4444; color: white; animation: pulse-red 2s infinite;" onclick="escalateCall('${lead.call_sid}')">🚨 Take Over</button>
...
async function viewConversation(sid, name, customerId) {
    try {
        ...
        const compliance = data.compliance || 'PENDING';
...
window.onscroll = function() {
    // Show button even with small scroll for better demo visibility
...
async function openApproveModal(sid) {
    try {
        ...
        let updates = lead.pending_updates || {};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('refresh-btn').onclick = () => {
        ...
        Promise.all([fetchKPIs(), fetchLeads()]).then(() => {
            btn.innerText = 'Manual Refresh';
        });
    };
});

async function escalateCall(sid) {
    if (!confirm("Are you sure you want to take over this call? The AI will stop and connect the user to your phone.")) return;
    
    try {
        const res = await fetch(`${API_BASE}/telephony/escalate/${sid}`, {
            method: 'POST',
            headers: { 'ngrok-skip-browser-warning': 'true' }
        });
        const data = await res.json();
        if (data.status === 'success') {
            alert("Transfer initiated! Please pick up your phone.");
            if (typeof fetchLeads === 'function') fetchLeads();
        } else {
            alert("Transfer failed: " + (data.detail || data.message));
        }
    } catch (e) {
...
let pendingResumeSid = null;

async function resumeAICall(sid) {
    pendingResumeSid = sid;
    document.getElementById('resume-summary').value = '';
    document.getElementById('resume-data').value = '';
    document.getElementById('resume-modal').classList.remove('hidden');
    
    document.getElementById('confirm-resume-btn').onclick = async () => {
        const summary = document.getElementById('resume-summary').value;
        const dataStr = document.getElementById('resume-data').value;
        let dataUpdates = null;
        
        if (dataStr.trim()) {
            try {
                dataUpdates = JSON.parse(dataStr);
            } catch (e) {
                alert("Invalid JSON format in structured updates.");
                return;
            }
        }
        
        try {
            const res = await fetch(`${API_BASE}/telephony/resume-ai/${sid}`, {
                method: 'POST',
                headers: { 
            ...
            const data = await res.json();
            if (data.status === 'success') {
                alert("AI has taken over the call again.");
                closeResumeModal();
                if (typeof fetchLeads === 'function') fetchLeads();
            } else {
            ...
            alert("System error during AI resumption.");
        }
    };
}
...
// Add CSS animation for pulsing button if not present
